...
pub trait Resource: PillTypeMapKey {
    ...
    fn initialize(&mut self, _engine: &mut Engine) -> Result<()> {
        Ok(())
    } // Called when resource is added to the engine, before adding it to storage
    ...
    fn deferred_update(&mut self, _engine: &mut Engine, _request: usize) -> Result<()> {
        Ok(())
    } // Called by DeferredUpdateSystem when request related to the resource is being processed
    fn destroy<H: PillSlotMapKey>(&mut self, _engine: &mut Engine, _self_handle: H) -> Result<()> {
        Ok(())
    } // Called when resource is being removed from the engine
}
