use crate::{
    ecs::{DeferredUpdateComponent, DeferredUpdateRequest},
    engine::Engine,
};
...
pub fn deferred_update_system(engine: &mut Engine) -> Result<()> {
    ...
    let mut deferred_update_manager = deferred_update_component
        .manager
        .0
        .lock()
        .expect("Critical: Mutex is blocked");

    // Create new empty queue
    let new_request_queue = VecDeque::<Box<dyn DeferredUpdateRequest + 'static>>::new();

    // Swap new queue with queue in component
    let mut request_queue = deferred_update_manager
        .request_queue
        .replace(new_request_queue)
        .expect("Critical: Queue is None");
