use crate::ecs::{
    Component, ComponentDestroyer, ComponentStorage, ConcreteComponentDestroyer, Entity,
    EntityHandle,
};
...
impl Scene {
        ...
        T: Component<Storage = ComponentStorage<T>>,
    {
        ...
        self.component_destroyers
            .entry(component_typeid)
            .or_insert_with(|| Box::new(component_destroyer));
    }
        ...
        A: Component<Storage = ComponentStorage<A>>,
    {
        ...
        let iterator = entities
            ...
            .map(move |(h, _)| {
                (
                    h,
                    Self::unsafe_mut_cast(storage_a)
                        .get_mut(h.0.index as usize)
                        .unwrap()
                        .as_mut()
                        .unwrap(),
                )
        ...
        B: Component<Storage = ComponentStorage<B>>,
    {
        // Generate filter bitmask
        let filter_bitmask =
            self.get_component_bitmask::<A>()? | self.get_component_bitmask::<B>()?;
            ...
            .map(move |(h, _)| {
                (
                    ...
                    Self::unsafe_mut_cast(storage_a)
                        ...
                        .as_mut()
                        .unwrap(),
                    Self::unsafe_mut_cast(storage_b)
                        .get_mut(h.0.index as usize)
                        .unwrap()
                        .as_mut()
                        .unwrap(),
        ...
        C: Component<Storage = ComponentStorage<C>>,
    {
        // Generate filter bitmask
        let filter_bitmask = self.get_component_bitmask::<A>()?
            | self.get_component_bitmask::<B>()?
            | self.get_component_bitmask::<C>()?;
            ...
            .map(move |(h, _)| {
                (
                    ...
                    Self::unsafe_mut_cast(storage_b)
                        ...
                        .as_mut()
                        .unwrap(),
                    Self::unsafe_mut_cast(storage_c)
                        .get_mut(h.0.index as usize)
                        .unwrap()
                        .as_mut()
                        .unwrap(),
